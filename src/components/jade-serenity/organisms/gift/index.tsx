import { AnimatedSection, Button } from '@/components/jade-serenity/atoms';
          ...
          <>
            {content.gift?.map((item, i) => (
              <AnimatedSection key={i}>
                <div className='bg-green-900/20 rounded-lg shadow-lg p-4 mb-6'>
                  <div className='flex flex-col justify-center items-center'>
                    <p className='text-md uppercase text-green-900 mb-1'>
                      Bank {item.bankName}
                    </p>
                    <button onClick={() => handleCopy(item.noRek)}>
                      <div className='flex items-center text-green-900 gap-1'>
                        <div>
                          <Copy />
                        </div>
                    ...
                    <p className='text-md text-green-900 mb-1'>
                      a.n {item.accName}
                    </p>
                  </div>
                </div>
    ...
    <div className='relative px-4 py-16 shadow-md z-10 bg-white'>
      ...
      <div className='flex items-center justify-center gap-2 mb-6'>
        <Button
          variant='outlined'
          className={cn(
            'rounded-full !bg-white border-green-900 hover:bg-green-900 text-green-900 flex gap-1 items-center justify-center',
            openGift === 'cashless' && '!bg-green-900 !text-white'
          )}
        ...
        <Button
          ...
          className={cn(
            'rounded-full !bg-white border-green-900 hover:bg-green-900 text-green-900 flex gap-1 items-center justify-center',
            openGift === 'gift' && '!bg-green-900 !text-white'
          )}

import {
  CountdownTimer,
  EventSection
} from '@/components/jade-serenity/molecules';
...
const Event = ({
  ...
  return (
    <div className='relative px-6 pt-32 pb-24 text-white text-center overflow-hidden trigger-rsvp'>
      ...
      />
      <div className='bg-white/30 backdrop-blur-sm relative z-10 flex flex-col rounded-[50px] shadow-lg py-4 px-5 text-green-900'>
        <AnimatedSection>
          ...
          <div className='mb-6'>
            <CountdownTimer
              targetDate={(content.event.resepsi1?.date as Date)?.toISOString()}
            />
          </div>
        </AnimatedSection>
        <AnimatedSection>
